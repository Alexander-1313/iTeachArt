...
public class SchedulerService {
    ...
    public void notifyUserAboutSubscribe(){
        ...
        for(User user: allUsers){
            ...
            }else if(LocalDate.now().minusDays(2L).isBefore(user.getSubscribeExpireDate())){
                mailService.sendEmail(user.getEmail(), StringConstant.SUBSCRIBE_SUBJECT, StringConstant.SOON_ENDING_TEXT);
                log.info("notify user with email={} about soon ending of subscribe...", user.getEmail());
            }else{
